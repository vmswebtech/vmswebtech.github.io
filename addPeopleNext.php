...
$messg=$messg1=$messg2=$messg3=$messg4=$messg5=$messg6=$messg7=$messg8=$messg9="";
 session_start();
		 ...
		 $sql="SELECT name,userId FROM volunteer where userId=".$id;
         $result = $conn->query($sql);
		 while ($row = mysqli_fetch_array($result)) {
			 $volunteer_name=$row['name'];
			 $volunteer_id=$row['userId'];
		 }
/*Cheacking request type*/
$name=$location=$description=$body_mark=$image="";
...
//chech already exist or not
    ...
    $sql = "SELECT * FROM community where name='$name' and location='$location' and body_mark='$body_mark', image='$image'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    $messg8="He/she is Already Added<br>";
	$uploadok = 0;
    }
...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
	...
	$sql = "INSERT INTO community (name, location, description, body_mark, image, volunteer_id, volunteer_name)
    VALUES ('$name', '$location', '$description','$body_mark', '$image', '$volunteer_id', '$volunteer_name')";
...
$messg7= "The file ". basename( $_FILES["image"]["name"]). " has been uploaded.<br>";
...
<a href="addPeople.php"><br><br><button class="button"><span>Go Back</span></button></div></a>
</body></html>